  ...
  const handleCaptureShopLocation = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('High accuracy GPS request timed out or failed, trying standard accuracy fallback:', highAccErr);
      ...
      if (lat !== null && lon !== null) {
        ...
        setFormData(prev => ({
          ...
          locationCapturedByUserId: currentUser?.id || 'admin'
        }));
      ...
      setGpsCaptureError(err.message || 'Unable to capture current GPS location. Please ensure Location permissions are allowed.');
    } finally {
                      ...
                      >
                        <div className="flex items-center gap-1.5">
                          ...
                          {cust.isGpsVerified && cust.latitude && cust.longitude ? (
                            <span className="text-[9px] text-teal-700 bg-teal-50 border border-teal-200 px-1.5 py-0.2 rounded font-bold flex items-center gap-0.5">
            ...
            <div className="sm:col-span-2 p-3.5 bg-slate-50 rounded-xl border border-slate-200 space-y-2.5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-teal-700 hover:bg-teal-800 text-white font-bold text-xs shadow-2xs inline-flex items-center gap-1.5 cursor-pointer disabled:opacity-50 transition-colors"
                  >
                    {isCapturingGps ? (
                      <>
              ...
              {formData.latitude && formData.longitude ? (
                ...
                <div className="bg-amber-50/70 p-2.5 rounded-lg border border-amber-200 text-amber-800 text-[11px] flex items-center gap-2">
                  ...
                  <span>No GPS location recorded yet. Click <strong>"Capture Shop Location"</strong> to set shop coordinates for seller visit verification.</span>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2 p-3.5 bg-slate-50 rounded-xl border border-slate-200 space-y-2.5">
                ...
                {formData.latitude && formData.longitude ? (
                  ...
                  <div className="bg-amber-50/70 p-2.5 rounded-lg border border-amber-200 text-amber-800 text-[11px] flex items-center gap-2">
                    <AlertCircle className="w-4 h-4 text-amber-600 shrink-0" />
                    <span>No GPS location recorded yet. Click <strong>"Capture Shop Location"</strong> to set geo-fence for seller visit verification.</span>
                  </div>
                )}
              </div>
