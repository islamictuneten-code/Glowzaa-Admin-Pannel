    ...
    <div className="space-y-5 pb-28 sm:pb-12">
      ...
      {activeTab === 'audit_logs' && (
        <div className="space-y-6">
          {/* Audit Report Summary & Copy Panel */}
          <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-900">Production Readiness & Data Integrity Audit Report</h3>
                <p className="text-xs text-slate-500">Comprehensive diagnostic findings across security, persistence, print, and mobile UI.</p>
              </div>
            </div>

            <button
              onClick={handleCopyAuditReport}
              className={`inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all shadow-xs cursor-pointer shrink-0 ${
                isAuditCopied
                  ? 'bg-emerald-600 text-white'
                  : 'bg-[#087F7A] hover:bg-[#066763] text-white'
              }`}
            >
              {isAuditCopied ? (
                <>
                  <CheckCircle2 className="w-4 h-4" />
                  <span>✓ Audit Report Copied</span>
                </>
              ) : (
                <>
          ...
          {/* Canonical Audit Report Text Viewer */}
          <div className="bg-slate-900 text-slate-100 p-4 sm:p-6 rounded-2xl border border-slate-800 shadow-inner font-mono text-xs overflow-x-auto max-h-96 leading-relaxed">
            <pre className="whitespace-pre-wrap">{fullAuditReportText}</pre>
          </div>
